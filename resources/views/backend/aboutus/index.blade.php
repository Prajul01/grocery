            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($data['rows'] as $i=>$setting)
                    <tr>
                        ...
                        <td> {{$i+1}}</td>
                        <td>{{$setting->shortdesc}}</td>
                        <td>{!! Str::limit( $setting->desc, 100)  !!}                          </td>

                        <td>
                            <img src="{{asset('uploads/images/aboutus/'.$setting->image)}}" class="image2" alt=""
                                 style="height: 100px; width: 100px; border: 15px solid white">

                        </td>
                        <td>
                            <a href="{{route('aboutus.show',$setting->id)}}" class="btn btn-sm btn-primary">
                                <i class="fa fa-eye"></i>

                            </a>
                            <a href="{{route('aboutus.edit',$setting->id)}}" class="btn btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
                            <form class="d-inline" action="{{route('aboutus.destroy',$setting->id)}}" method="post">
