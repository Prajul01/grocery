            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($data['row'] as $i=>$cat)
                    <tr>
                        ...
                        <td>{{$i+1}} </td>
                        <td>{{$cat->User->name}}</td>
                        <td>{{$cat->Products->name}}</td>
                        <td>{{$cat->quantity}}</td>
                        <td><img src="{{asset('uploads/images/featureimage/'.$cat->Products->feature_image)}}" class="image2" alt="" style="height: 100px; width: 100px; " >
                        </td>
                        <td>{{$cat->created_at}}</td>
                        <td>
                            <input data-id="{{$cat->id}}" class="toggle-class" type="checkbox" data-onstyle="success"
                                   data-offstyle="danger" data-toggle="toggle" data-on="Delivered" data-off="Pending"
                                {{ $cat->status ? 'checked' : '' }}>
